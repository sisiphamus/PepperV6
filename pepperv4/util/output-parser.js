...
export function parseOutputSpec(raw) {
...
function normaliseMemoryEntry(entry) {
  ...
  if (entry.file) {
    // e.g. "memory/skills/windows-system.md" → name: "windows-system", category: "skill"
...
export function parseTeacherResult(raw) {
  const parsed = extractJSON(raw);
...
export function parseLearnerResult(raw) {
  const parsed = extractJSON(raw);
